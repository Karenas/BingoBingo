...
//add by leiyi to remove .mtr torrent file
		File(group->getFirstFilePath()).remove();
          }
#ifdef ENABLE_BITTORRENT
		...
		for(auto & f : dctx->getFileEntries())
		{
//			A2_LOG_INFO(fmt("----------------------Checking %s to trim tail .part-----------", f->getPath().c_str()));
			if(util::iendsWith(f->getPath(), ".part"))
			{
				File(f->getPath()).renameTo(f->getPath().substr(0, f->getPath().length()-sizeof(".part")+1));
				A2_LOG_INFO(fmt("Renaming %s to trim tail .part-----------", f->getPath().c_str()));
			}
		}
#endif // ENABLE_BITTORRENT
        } else {
