...
size_t AbstractDiskWriter::m_nL3BufLen = 0;
unsigned char* AbstractDiskWriter::m_pL3Buf = NULL;

AbstractDiskWriter::AbstractDiskWriter(const std::string& filename, uint8_t aKey)
  : filename_(filename),
    m_nL3Key(aKey),
    fd_(A2_BAD_FD),
#ifdef __MINGW32__
...
//插入用户加密处理过程。
	if(!m_pL3Buf || m_nL3BufLen < len)
	{
		m_pL3Buf = (unsigned char*)realloc(m_pL3Buf, len);
		m_nL3BufLen = len;
	}
	if(!m_pL3Buf) return 0;	//Failed because of memory exhaust and return 0;

    unsigned char* p = m_pL3Buf;
    ...
    while((size_t)writtenLength < len) {
#ifdef __MINGW32__
      DWORD nwrite;
      if(WriteFile(fd_, m_pL3Buf/*data*/+writtenLength, len-writtenLength, &nwrite, 0)) {
      ...
      while((ret = write(fd_, m_pL3Buf/*data*/+writtenLength, len-writtenLength)) == -1 &&
            errno == EINTR);
      if(ret == -1) {
        return -1;
      }
                                             ...
                                             int64_t offset)
{
  if(mapaddr_) {
	...
	unsigned char* p = (unsigned char*)data; for(size_t i=0;i<len;i++) *p++ ^= m_nL3Key;
