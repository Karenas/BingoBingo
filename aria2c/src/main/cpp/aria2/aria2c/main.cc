...
//20140520 增加RPC绑定端口记录文件功能
//生成记录文件的路径：DIR_OF_ARIA2C/../aria2c.port，或者 ../aria2c.port
	std::string rpcPortFile(argv[0]);
	size_t nPos = rpcPortFile.find_last_of('/');
	if(nPos == std::string::npos)
		rpcPortFile = "../aria2c.port";
	else
		rpcPortFile = rpcPortFile.substr(0, nPos+1)+"../aria2c.port";
	RpcPortFilePath = (char*)malloc(rpcPortFile.length()+30);
	strcpy(RpcPortFilePath, rpcPortFile.c_str());
	RpcPortFilePath[rpcPortFile.length()]=0;
    A2_LOG_DEBUG(fmt("RPC port record file is: %s", RpcPortFilePath));

  error_code::Value exitStatus = error_code::FINISHED;
  if(context.reqinfo) {
    exitStatus = context.reqinfo->execute();
  }
  free(RpcPortFilePath);
  return exitStatus;
}
