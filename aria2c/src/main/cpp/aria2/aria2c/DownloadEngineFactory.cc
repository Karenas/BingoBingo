...
(Option* op, std::vector<std::shared_ptr<RequestGroup>> requestGroups)
{
  ...
  if(op->getAsBool(PREF_ENABLE_RPC)) {
    ...
    File rpcPortFile = File(RpcPortFilePath);
    if(rpcPortFile.exists()) {
        A2_LOG_NOTICE(fmt("Removing old RPC port record file :%s",RpcPortFilePath));
    	rpcPortFile.remove();
    }
    ...
    for(size_t i = 0; i < familiesLength; ++i) {
      ...
      /*if(httpListenCommand->bindPort(op->getAsInt(PREF_RPC_LISTEN_PORT))){
        ...
        ok = true;
      } 20140520 改为从配置端口开始寻找一个可用端口*/
      uint16_t nStartPort = op->getAsInt(PREF_RPC_LISTEN_PORT);
      uint16_t nTry = 0;
	  #define USER_BIND_MIN_PORT 1024
      while(!ok && nTry++ < 65535-USER_BIND_MIN_PORT) {
		if( nStartPort < USER_BIND_MIN_PORT )
			nStartPort = USER_BIND_MIN_PORT;
        if( httpListenCommand->bindPort(nStartPort) ) {
          A2_LOG_INFO(fmt("Save RPC port %d to file %s.", nStartPort, RpcPortFilePath));
          e->addCommand(std::move(httpListenCommand));
          std::shared_ptr<OutputFile> fpp_;
          fpp_.reset(new BufferedFile(RpcPortFilePath, BufferedFile::WRITE));
          fpp_->printf("%d\n", nStartPort);
          fpp_.reset();
          ok = true;
        }
