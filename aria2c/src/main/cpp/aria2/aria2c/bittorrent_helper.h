...
//add by leiyi to parse subkey from sqlite
#define USE_SQLITE
#ifndef USE_SQLITE
//add by leiyi to parse subkey from SFileC.xml or PID[_EID].lll
...
#ifndef USE_SQLITE
class L3SubfileParserStateMachine : public ParserStateMachine {
public:
	L3SubfileParserStateMachine();
	virtual ~L3SubfileParserStateMachine();

	virtual bool needsCharactersBuffering() const CXX11_OVERRIDE;
	virtual bool finished() const CXX11_OVERRIDE;
	virtual void beginElement(const char* localname, const char* prefix, const char* nsUri, const std::vector<XmlAttr>& attrs) CXX11_OVERRIDE;
	virtual void endElement(const char* localname, const char* prefix, const char* nsUri, std::string characters) CXX11_OVERRIDE;
	virtual void reset() CXX11_OVERRIDE;

	void	SetPid(uint32_t nPid);
	void	SetEid(uint32_t nEid);
	uint8_t GetL3Key();
private:
	uint32_t m_nPid;
	uint32_t m_nEid;
