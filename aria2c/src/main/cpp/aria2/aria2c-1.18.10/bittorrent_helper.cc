...
namespace aria2 {
#ifdef USE_SQLITE
static int sqlSk;	//sk 结果
static int sqlCallback(void *NotUsed, int argc, char **argv, char **azColName)
{
/*	A2_LOG_INFO(fmt("sqlCallback with argc=%d", argc)); //this should be the line num of the result
	int i;
	for(i=0; i<argc; i++)
		{
		A2_LOG_INFO(fmt("sqlCallback: %d:azColName=%s, argv=%s", i, azColName[i], argv[i] ? argv[i] : "NULL"));	//e.g.: sqlCallback: 0:azColName=sk,
		}
	return 0; */

	if(argc == 1 )
		{
		if( strcmp(azColName[0],"sk") == 0 )
			for(int j=0; j<strlen(argv[0]); j++ )
				sqlSk = sqlSk * 10 + argv[0][j] - '0';	//int64 will only left low int32, no problem
		}
//	A2_LOG_INFO(fmt("%s to %d", argv[0], sqlSk));	//e.g. 576226881783149937 to -961071759, test OK !
	return 0;
}
#define CHECK_RC(rc,szInfo,szErrMsg,db) if(rc!=SQLITE_OK) \
           {printf("%s error!/n%s/n",szInfo,szErrMsg);\
           sqlite3_free(szErrMsg);         \
           sqlite3_close(db);              \
           return 0;}
int doSql(char *szSql)
{
	sqlSk = 0;	//reset to 0

 	sqlite3 *ppDb;

	int rc = sqlite3_open("../lalala.db"/* Database filename (UTF-8)*/, &ppDb/*OUT: SQLite db handle */);
	char *szErrMsg = NULL;
	CHECK_RC(rc, "open database", szErrMsg, ppDb);
//	rc = sqlite3_exec(ppDb, szSql, 0, 0, &szErrMsg);
//	CHECK_RC(rc, "do sql", szErrMsg, ppDb);
	rc = sqlite3_exec(ppDb, szSql, sqlCallback, 0, &szErrMsg);
	CHECK_RC(rc, "do sql with callback", szErrMsg, ppDb);
//	A2_LOG_INFO(fmt("Do sql return: %s", szErrMsg));
...
#else
L3SubfileParserStateMachine::L3SubfileParserStateMachine()
{
	m_nL3Key = 0;
	m_nEid = 0;
}
...
bool L3SubfileParserStateMachine::needsCharactersBuffering() const
{
  return true;
}

void L3SubfileParserStateMachine::reset()
{
}

bool L3SubfileParserStateMachine::finished() const
{
  return true;
}

void L3SubfileParserStateMachine::beginElement(const char* localname, const char* prefix, const char* nsUri, const std::vector<XmlAttr>& attrs)
{
	int nAttrNum = attrs.size();
	const char* strEleName = m_nEid ? "Entry" : "Sd";
	if( strcasecmp(localname, strEleName)==0 )
	{
		int indexId=-1, indexSubKey=-1;
		for(int i=0; i<nAttrNum; i++)
		{
			if(strcasecmp(attrs[i].localname, "id")==0)
				indexId = i;
			else if(strcasecmp(attrs[i].localname, "sk")==0)
				indexSubKey = i;
		}
		int id = m_nEid ? m_nEid : m_nPid;
		if(indexId>-1 && indexSubKey >-1 && atoi(attrs[indexId].value) == id)
		{
			A2_LOG_INFO(fmt("Found subkey for %d_%d is %s", m_nPid, m_nEid, attrs[indexSubKey].value));
			uint64_t nKey64 = 0;
			const char* p = attrs[indexSubKey].value;
			int nKeyStrLen = strlen(p);
			for(int j=0; j<nKeyStrLen; j++)
			{
				char c=p[j];
				uint8_t digit;
				...
				else { assert(c == '/');	digit = 63; }
				nKey64 = (nKey64<<6) + digit;
			}
			m_nL3Key = (uint8_t)nKey64;
		}
	}
}

void L3SubfileParserStateMachine::endElement(const char* localname, const char* prefix, const char* nsUri, std::string characters)
{
}

void	L3SubfileParserStateMachine::SetPid(uint32_t nPid)
{
	m_nPid = nPid;
}

void	L3SubfileParserStateMachine::SetEid(uint32_t nEid)
{
	m_nEid = nEid;
}

uint8_t L3SubfileParserStateMachine::GetL3Key()
{
	return m_nL3Key;
}
#endif	//end of #ifndef USE_SQLITE

//*字符串切割
void splitString(const std::string& inputStr, const std::string& delimStr, std::vector<std::string>& outStrVec, bool isSaveZero=true)
{
	const int len = inputStr.length();	//std::string::size_type 是 unsigned ，不能用于以下机制
	int lastPos = -delimStr.length();
	while(lastPos < len)
	{
		int thisPos = lastPos + delimStr.length();
		lastPos = inputStr.find_first_of(delimStr, thisPos);
		if ( lastPos == (int)std::string::npos )
			lastPos = len;
		if(lastPos > thisPos || isSaveZero)
			outStrVec.push_back(inputStr.substr(thisPos, lastPos - thisPos));	//假定 substr(thisPos, 0) 可以获得0长度的一个子串
	}
}
 ...
 const std::vector<std::string>& urlList)
{
  ...
  std::vector<std::string> vecPidEid;
  if(filesList) {
    ...
    for(auto & f : *filesList) {
      ...
      //20140507 by leiyi to add .part after uncompleted file name
      utf8Path += ".part";
	...
	splitString(utf8Name, "_", vecPidEid, false);
  } else {
    // single-file mode;
    ...
    fileEntries.push_back(fileEntry);
	splitString(utf8Name, "_", vecPidEid, false);
	vecPidEid.pop_back();	//单文件时, torrent 的 name 比多文件的 pid[_eid] 之后多 _mfn.mpl，因此去掉 mfn.mpl
  }
  ...
  if(torrent->mode == BT_FILE_MODE_MULTI) {
...
#ifdef USE_SQLITE
	unsigned int pid=0, eid=0;
	if(vecPidEid.size()>0)
		pid = atoi(vecPidEid[0].c_str());
	if(vecPidEid.size()>1)
		eid = atoi(vecPidEid[1].c_str());
	char sql[128];	//sql centense
	if (eid > 0)	//P2 or P3
		sprintf(sql, "select sk from PEFile where pid=%d and eid=%d", pid, eid);
	else	//P1
		sprintf(sql, "select sk from SFile where id=%d", pid);
	uint8_t nKey = doSql(sql);
#else
	L3SubfileParserStateMachine psm;
	if(vecPidEid.size()>0)
		psm.SetPid(atoi(vecPidEid[0].c_str()));
	std::string xmlFName;
	if(vecPidEid.size()>1)
	{
		psm.SetEid(atoi(vecPidEid[1].c_str()));
		xmlFName = vecPidEid[0] + ".lll";
	}
	else
	{
		psm.SetEid(0);
	...
	/*20140506注：当加 -D 运行时，当前目录改为 / ，XML文件无法打开而卡在 [INFO] [BtPostDownloadHandler.cc:69] Generating RequestGroups for Torrent file ...而没有进入到
	 [INFO] [bittorrent_helper.cc:111] Found subkey for 2010_0 is QDqhUCitOg 时，在此处加观测动作，发现卡在 xml::parseFile(xmlFName, &psm) 过程，
	 且 top 命令显示CPU占用 23% 左右。在 XmlParser.cc 文件的 parseFile() 的 while {} 部分中添加10次重试限制以后，克服此bug
	A2_LOG_DEBUG(fmt("Parsing %s",xmlFName.c_str()));	*/
//在两级目录中尝试查找 L3 订购文件。目前 A_a_1 的文件布局是 xml 文件在 L3HOME/下，而 .mpl 在 L3HOME/MFiles/ 子目录下。aria2c 将在 L3HOME/MFiles 下启动
	if(!xml::parseFile("../"+xmlFName, &psm))
		xml::parseFile(xmlFName, &psm);
//	A2_LOG_DEBUG(fmt("Parse %s end.",xmlFName.c_str()));
	uint8_t nKey = psm.GetL3Key();
#endif
	if(nKey)
		ctx->SetL3Key(nKey);
	else
		A2_LOG_INFO("This is a normal torrent, not for lalala");
}
} // namespace
