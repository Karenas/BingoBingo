...
std::vector<OptionHandler*> OptionHandlerFactory::createOptionHandlers()
{
  ...
  }
  {
    OptionHandler* op(new BooleanOptionHandler
                      (PREF_DISABLE_IPV6,
                       ...
                       A2_V_TRUE,	//modified by leiyi to disable ipv6 by default	//A2_V_FALSE,
...
#endif // HAVE_MMAP || __MINGW32__
  {
    OptionHandler* op(new BooleanOptionHandler
                      (PREF_ENABLE_RPC,
                       TEXT_ENABLE_RPC,
                       A2_V_TRUE,	//20140506 modified by leiyi to enable RPC by default, //A2_V_FALSE,
                       OptionHandler::OPT_ARG));
    op->addTag(TAG_RPC);
    handlers.push_back(op);
  }
  ...
  }
  {
    OptionHandler* op(new ParameterOptionHandler
                      (PREF_FILE_ALLOCATION,
                       ...
                       V_NONE, //modified by leiyi to run V_NONE by default //V_PREALLOC,
  ...
  }
  {
    OptionHandler* op(new DefaultOptionHandler
                      (PREF_LOG,
                       TEXT_LOG,
                       "aria2c.log",	//modified to log, and file name aria2c.log  //NO_DEFAULT_VALUE,
                       PATH_TO_FILE_STDOUT,
                       OptionHandler::REQ_ARG,
                       'l'));
    op->addTag(TAG_BASIC);
  ...
  }
  {
    OptionHandler* op(new NumberOptionHandler
                      (PREF_RPC_LISTEN_PORT,
                       ...
                       "8090", //modified by leiyi to change default RPC port to 8090 //"6800",
                       1024, UINT16_MAX));
    op->addTag(TAG_RPC);
    handlers.push_back(op);
  }
  ...
  }
  {
    OptionHandler* op(new BooleanOptionHandler
                      (PREF_BT_ENABLE_LPD,
                       TEXT_BT_ENABLE_LPD,
                       A2_V_TRUE,	//modified by leiyi to enable LPD default A2_V_FALSE,
                       OptionHandler::OPT_ARG));
    op->addTag(TAG_BITTORRENT);
    op->setInitialOption(true);
    op->setChangeGlobalOption(true);
  ...
  }
  {
    OptionHandler* op(new BooleanOptionHandler
                      (PREF_ENABLE_DHT,
                       TEXT_ENABLE_DHT,
        		       A2_V_TRUE,	//temporary changed to 1 for Guiwenjun to test UDP tracker
//                       A2_V_FALSE,	//modified by leiyi to disable DHT by default //A2_V_TRUE,
                       OptionHandler::OPT_ARG));
    op->addTag(TAG_BASIC);
    op->addTag(TAG_BITTORRENT);
    handlers.push_back(op);
  ...
  }
  {
    OptionHandler* op(new IntegerRangeOptionHandler
                      (PREF_LISTEN_PORT,
                       TEXT_LISTEN_PORT,
                       "8088-8099",	//modified by leiyi to change peer port to 8088 //"6881-6999",
                       1024, UINT16_MAX));
    op->addTag(TAG_BASIC);
    op->addTag(TAG_BITTORRENT);
    handlers.push_back(op);
