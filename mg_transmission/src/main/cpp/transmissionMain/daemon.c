...
#ifdef DEBUG
void __log(char * Value)
{
    FILE *fp;

    fp = fopen ("/sdcard/lalalaFile/bt", "a+");
    fprintf(fp, "%s\n", Value);
    fclose(fp);
}
#endif
...
getConfigDir (int argc, const char ** argv)
{
    ...
    while ((c = tr_getopt (getUsage (), argc, argv, options, &optarg))) {
        if (c == 'g') {
#ifndef DEBUG   //Release版本中，借用这个参数来设置HomeDir为lalala文件的路径
            setHomeDir(optarg);
#else
            configDir = optarg;
#endif
...
main (int argc, char ** argv)
{
    ...
    daemonPid = lalala_getDaemonPID(/*daemonPidStr*/);
    if (daemonPid != 0) {
        //daemon already exist
        c = kill(daemonPid, SIGKILL);
        if (c != 0)
            exit(0);    //Kill the old daemon failed, keep using the old one.
    }
	...
	nice(15);		//Lower the running priority
