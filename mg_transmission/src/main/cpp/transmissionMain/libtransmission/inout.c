 ...
 *
 * $Id: inout.c 13868 2013-01-25 23:34:20Z jordan $
 */
                  ...
                  size_t             buflen)
{
  ...
  //Wenyu, Following 4 lines are for Media File En/Decryption
  int Enkey;
  int i;
  unsigned char Factor, *tempbuf;
  //Wenyu, Code for Media File En/Decryption finished
  ...
  if (!err)
    {
	  ...
	  //######### Wenyu Begin
            ...
            if ( tor->enkeyNotSet) {  //enkey is not set yet
              tor->enkey = lalala_getEnkey(tor->info.name);     //Set enkey
              tor->enkeyNotSet = false;
            }

            if (tor->enkey != 0) {
                Factor = (unsigned char) (&(tor->enkey))[0];
                //This is to do the personal decryption after read from file
	  ...
	  //######### Wenyu Begin
          ...
          if (tor->enkey != 0) {
            Factor = (unsigned char) (&(tor->enkey))[0];
            //This is to do personal Encryption before write to file
            tempbuf = (unsigned char *) buf;
            for (i = 0; i < buflen; i ++)
              tempbuf[i] ^= Factor;
          }
