...
#define PACKAGE_DATA_DIR "/sdcard/lalalaFile"
/***
****  THREADS
***/
...
#ifdef ANDROID  //用于在Android平台设置Lalala文件的路径
static char * lalalahome = NULL;        //为全局变量，在setHomeDir和getHomeDir中都要用到
void setHomeDir (char * homeDir)
{

  lalalahome = tr_strdup (homeDir);
  return;
}
#endif

#ifdef ANDROID  ///在Android平台，需要在lalala.c中使用这个函数，所以不能定义为static
const char *
#else
static const char *
#endif
getHomeDir (void)
{
  ...
  if (!home)
    {
#ifdef ANDROID
        //在Android平台，若没有从daemoc.c中设置HomeDir，则直接将homeDir设置为 /sdcard/lalalaFile
        if (! lalalahome)       //若没有从daemoc.c中设置HomeDir，则直接将homeDir设置为 /sdcard/lalalaFile
            home = tr_strdup ("/sdcard/lalalaFile");
        else                    //否则，设置HomeDir为lalala文件所在路径
            home = lalalahome;
#else
...
tr_getDefaultConfigDir (const char * appname)
{
  ...
  if (!s)
    {
#ifdef ANDROID
      //ANDROID平台，定义为HomeDir下的transmission目录
      s = tr_buildPath (getHomeDir (), "transmission", NULL);
#else
...
tr_getDefaultDownloadDir (void)
{
  ...
  if (user_dir == NULL)
    {
#ifdef ANDROID
        //Android平台,直接设置为HomeDir下的MFiler下的MFiles目录
        user_dir = tr_buildPath (getHomeDir (), "MFiles", NULL);
#else  
