...
torrentInit (tr_torrent * tor, const tr_ctor * ctor)
{
  ...
  if (isNewTorrent)
    ...
    {
//Following Code are added by Wenyu to support PC Assistant File transfer.
//When PC Assistand move a Media File to Android Client, it must also delete the .part file for that torrent.
        if (tor->completeness != TR_SEED) {
                struct stat st;
                char * filename, *filename1;

                filename = tr_strdup_printf ("%s/%s", tor->downloadDir, tor->info.name);
                if (! stat (filename, &st)) {           //Media File exist, treat it as a New Torrent
                        tor->startAfterVerify = doStart;
                        tr_torrentVerify (tor, NULL, NULL);
                } else          //Media File doesn't exist, use the old code to start the torrent.
                        tr_torrentStart (tor);
                tr_free (filename);
        } else  //It's an old torrent with download finished (In seeding status), use the old code.
//Wenyu End
      tr_torrentStart (tor);
    }
