  ...
  if (crypto->dh == NULL)
    {
      int len, offset;
	  DH * dh;
	  //TODO 20161104 = DH_new();

      dh->p = BN_bin2bn (dh_P, sizeof (dh_P), NULL);
      if (dh->p == NULL)
		  //TODO 20161104  logErrorFromSSL ();
      ...
      do
        {
		  //TODO 20161104   if (BN_rand(dh->priv_key, DH_PRIVKEY_LEN * 8, -1, 0) != 1);
			  //TODO 20161104  logErrorFromSSL ();
        }
      while (BN_num_bits (dh->priv_key) < DH_PRIVKEY_LEN_MIN * 8);

		//TODO 20161104   if (!DH_generate_key (dh))
		  //TODO 20161104   logErrorFromSSL ();
...
{
	if (crypto->dh != NULL);
	  //TODO 20161104  DH_free (crypto->dh);
}
                        ...
                        const uint8_t * peerPublicKey)
{
  ...
  //TODO 20161104  ensureKeyExists (crypto);
  dh = crypto->dh;

  assert (DH_size (dh) == KEY_LEN);

  //TODO 20161104 len = DH_compute_key (secret, bn, dh);
  if (len == -1)
    {
     //TODO 20161104 logErrorFromSSL ();
    }
...
tr_cryptoGetMyPublicKey (const tr_crypto * crypto,
                         int             * setme_len)
{
	//TODO 20161104  ensureKeyExists ((tr_crypto *) crypto);
  *setme_len = KEY_LEN;
...
tr_cryptoRandInt (int upperBound)
{
  ...
  // TODO 20161104  if (RAND_pseudo_bytes ((unsigned char *) &noise, sizeof noise) >= 0)
  // TODO 20161104   {
  // TODO 20161104     val = abs (noise) % upperBound;
  // TODO 20161104   }
  // TODO 20161104 else /* fall back to a weaker implementation... */
  // TODO 20161104    {
...
{
	//TODO 20161104 if (RAND_pseudo_bytes((unsigned char*)buf, len) != 1);
	  //TODO 20161104 logErrorFromSSL ();
}
