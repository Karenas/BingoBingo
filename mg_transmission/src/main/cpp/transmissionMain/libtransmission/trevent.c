...
#ifdef ANDROID		//For Android, get DNS server IP address from system call instead of /etc/resolv.conf file
    char ipaddr[16];
	__system_property_get("net.dns1", ipaddr);
    eh->session->evdns_base = evdns_base_new (base, false);
    evdns_base_nameserver_ip_add(eh->session->evdns_base, ipaddr);
#else
