...
tr_sessionGetDefaultSettings (tr_variant * d)
{
  ...
  tr_variantDictAddBool (d, TR_KEY_dht_enabled,                     false);        //Lalala is private track, don't need it.       true);
  tr_variantDictAddBool (d, TR_KEY_utp_enabled,                     true);
  tr_variantDictAddBool (d, TR_KEY_lpd_enabled,                     true);
  tr_variantDictAddStr  (d, TR_KEY_download_dir,                    tr_getDefaultDownloadDir ());
  tr_variantDictAddInt  (d, TR_KEY_speed_limit_down,                1000);			//Set the download speed limit to 1M
  tr_variantDictAddBool (d, TR_KEY_speed_limit_down_enabled,        true);			//Enable the download speed limit
  tr_variantDictAddInt  (d, TR_KEY_encryption,                      0);             //Lalalal don't need encryption. TR_DEFAULT_ENCRYPTION);
  ...
  tr_variantDictAddInt  (d, TR_KEY_download_queue_size,             3);		//5);
...
#ifdef DEBUG    //Debug 版本允许任何连接，Release版本只允许本机的连接
...
//以下 RPC 接口运行参数改为固定赋值方式，而不是从配置文件中读
  tr_variantDictAddBool (d, TR_KEY_rpc_authentication_required,  false); //tr_sessionIsRPCPasswordEnabled (s));
  tr_variantDictAddStr  (d, TR_KEY_rpc_bind_address,             tr_sessionGetRPCBindAddress (s));
  tr_variantDictAddBool (d, TR_KEY_rpc_enabled,                  true); //tr_sessionIsRPCEnabled (s));
  tr_variantDictAddStr  (d, TR_KEY_rpc_password,                 ""); //tr_sessionGetRPCPassword (s));
  tr_variantDictAddInt  (d, TR_KEY_rpc_port,                     8090); //tr_sessionGetRPCPort (s));
...
//注掉上述2行，改为无条件设置仅允许本机连接
#ifndef DEBUG  //TODO 20161110
#define DEBUG
#endif
#ifdef DEBUG
  tr_variantDictAddBool (d, TR_KEY_rpc_whitelist_enabled,        false);
...
#ifdef DEBUG   //在release版本中，所有的参数均取自default值，不允许通过命令行或文件来修改，所以不需要下面的代码
