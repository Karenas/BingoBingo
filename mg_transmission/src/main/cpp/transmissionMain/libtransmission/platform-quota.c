...
#include <sys/statfs.h>	//For Android Platform only
#include "lalala.h"	//For Android, to implementation function setmntent() and endmntent()
 ...
 #else
  #include <linux/quota.h>	//#include <sys/quota.h> /* quotactl() */
 #endif
...
/*  struct dqblk dq;
  ...
  return -1;	//Don't need this function on Android platform
}
...
tr_device_info_get_free_space (const struct tr_device_info * info)
{
  ...
  if ((info == NULL) || (info->path == NULL))
    ...
    {
      //Don't need for Andoird		free_space = tr_getQuotaFreeSpace (info);
