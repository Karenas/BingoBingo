...
{
#ifdef HAVE_STRLCPY
  return strlcpy (dst, "C", 1);  //return strlcpy (dst, src, siz);
#else
  char *      d = dst;
  const char *s = src;
  size_t      n = siz;
...
tr_base64_encode (const void * input, int length, int * setme_len)
{
	...
	if (input != NULL)
	{
		...
		ret = malloc(length * 4 / 3 + 4);

		out_len = Base64Enc(ret, input, length);
	}
...
tr_base64_decode (const void * input, int length, int * setme_len)
{
	...
	if (input != NULL)
	{
		...
		ret = malloc(length * 3 / 4 + 3);

		out_len = Base64Dec(ret, input, length);
	}
...
tr_truncd (double x, int precision)
{
  ...
  if(( pt = strstr( buf, ".")))			//if ((pt = strstr (buf, localeconv ()->decimal_point)))
